...
export async function searchAddresses(q: string, limit = 10): Promise<SearchResponse> {
  const res = await fetch(`/api/v1/search?q=${encodeURIComponent(q)}&limit=${limit}`);
...
export async function lookupByEsiId(esiId: string): Promise<LookupResponse> {
  ...
  if (!res.ok && res.status !== 404) throw new Error(`Lookup failed: ${res.status}`);
